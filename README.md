...
·	`f1_orange.tab` — the dataset in Orange format (12,228 rows, 8 columns)
·	`f1_assignment.ows` — the pre-built workflow
Quick Start
1.	Open Orange 3
2.	File → Open → select `f1_assignment.ows`
3.	Double-click the "F1 Dataset" File widget → click the folder icon → choose `f1_orange.tab` → Apply
4.	All widgets will populate automatically
...
Target variable	`result_class` (class role)
...
·	In Orange's File widget, verify: 8 columns visible, `result_class` shows Role = class
...
Box Plot
·	Double-click → set variable to `grid`, group by `result_class`
·	Repeat for `laps`
